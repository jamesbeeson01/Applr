...
- **`dplyr (>= 1.0.0)`** — `geom_slice.R` calls `select()`, `right_join()`, and `rename()` (and `%>%` was already imported in `NAMESPACE`), but `dplyr` was never listed in `DESCRIPTION` and those three verbs were not imported. It worked only because `dplyr` happened to be attached during `devtools::load_all()`; a built package would fail `R CMD check` ("no visible global function definition for 'select'"). Added `@importFrom dplyr select right_join rename` to the `StatSlice` roxygen block so `NAMESPACE` regenerates correctly. `1.0.0` (2020) is a safe floor — all three verbs and the tidyselect negation (`!x`) long predate it.
