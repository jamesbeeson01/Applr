...
- [ ] Add a "pretty title" to test cases
  - [ ] implement in report
...
- [ ] Separate multi-plot test files
- [ ] Test `scatter_3d`
...
- [ ] Migrate fully to snake_case later (e.g. `xaxis` → `x_axis`, `color`/`colors`), dropping the current public-argument exceptions in `decisions.Rmd`. Requires: (1) a deprecation plan for renamed public arguments (aliases + warning/`lifecycle`, removal timeline), and (2) updating the naming-conventions standard once settled.
...
- [ ] Allow specifying impute method — min / max / median (numeric) — instead of the default mean; add `gs_` test cases for each when implemented. (Current impute types — numeric -> mean, factor/character -> mode — are covered by `gs_17`/`gs_18`.)
