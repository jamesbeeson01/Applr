...
A small R package for visualizing linear models. Plots linear models from 
simple lines to high-dimensional squiggles all with the same level of difficulty.

Fit a model with `lm()`, then let Applr draw it. Explore easily with `autoplot()`, 
or have fine-grained control with `geom_slice()`: a ggplot2 layer baked with 
confidence bands, projections, and group/facet support for *your* fitted model, 
plotting it the way `geom_smooth()` draws its own. Most importantly, you can specify 
predictor values to control what "slice" of high-dimensional space is shown, or let 
`geom_slice` pick for you. See its choice with console messages, or show it with 
`geom_slice_subtitle()` and its friends.

Also includes base-R helpers (`slice_2d()`), interactive 3-D surfaces (`scatter_3d()`), 
LaTeX generator (`lm_latex()`), and a simple diagnostic helper (`diagnose()`).
...
```{r quickstart-autoplot, fig.keep = "first", fig.show = "hold", fig.alt = "autoplot() of an lm: scatter plot with the fitted slice line"}
library(Applr)
...
```{r facets, fig.width = 7, fig.alt = "geom_slice() drawing the model's line in every facet"}
library(ggplot2)
...
```{r interval, fig.alt = "Slice line with a prediction-interval ribbon"}
model <- lm(mpg ~ disp + hp, data = mtcars)
...
```{r back-transform, fig.alt = "Back-transformed slice of a log-response model on the raw mpg axis"}
...
```{r grouping, fig.alt = "One slice line per cyl group, colored to match the points"}
...
```{r full-range, fig.alt = "Slice lines extended across the full panel x range with full_range = TRUE"}
ggplot(mtcars, aes(disp, mpg, color = factor(cyl))) +
  geom_point() +
...
```{r slice-text, fig.width = 7.5, fig.alt = "Slice lines labeled at their right ends with hp: 66, hp: 123, hp: 335"}
library(ggplot2)
model <- lm(mpg ~ disp + hp, data = mtcars)
...
predictor goes on the x-axis (override with `mapping = aes(x = ...)`), and
everything in `...` is passed on to `geom_slice()`. The result is a regular
ggplot, so extend it with `+` as usual.

```{r autoplot, fig.keep = "last", fig.alt = "autoplot() with a confidence interval and a custom title"}
...
`aes()` (entries named `x` or `y` replace the chosen axes, and aesthetics
such as `color` are inherited by `geom_slice()`, so a model variable draws
one line per group). Choose the x-axis with `mapping = aes(x = ...)`.

```{r autoplot-mapping, fig.alt = "autoplot() with an x-axis override and a color mapping"}
autoplot(lm(mpg ~ disp + hp + factor(cyl), data = mtcars),
         mapping = aes(x = hp, color = factor(cyl)))
```

```{r autoplot-3d, screenshot.opts = list(vwidth = 800, vheight = 550, delay = 2), fig.alt = "Interactive 3-D scatter with the fitted regression surface (static snapshot)"}
...
```{r add-slice-2d-mismatch, eval = FALSE}
...
```{r scatter-3d-transformed, eval = FALSE}
...
```{r theme-lc, fig.alt = "Scatter plot drawn with theme_lc()"}
library(ggplot2)
