...
`geom_smooth()` draws its own, base-R helpers (`slice_2d()`), interactive
slider exploration (`slice_explore()`), 3-D surfaces (`scatter_3d()`), and
friends.
...
```{r autoplot-data-mapping, fig.alt = "autoplot() with a custom data frame and color mapping"}
...
```{r autoplot-sliders, screenshot.opts = list(vwidth = 800, vheight = 700, delay = 2), fig.alt = "Interactive slider view: scatter with a slice line and one slider per hidden predictor (static snapshot)"}
...
smoothly, moving the line to that slice of the model as you go. Each slider
starts at the value the static plot would impute, so the starting view is
the plot above. `slider_breaks = list(hp = c(66, 150, 335))` gives
deliberate steps instead; `sliders = factor(cyl)` steps a numeric predictor
through its observed values; see `?slice_explore`.)*
