...
```{r quickstart-autoplot, fig.keep = "first", fig.show = "hold", fig.alt = "autoplot() of an lm: scatter plot with the fitted slice line", results = "hide"}
library(Applr)
...
```{r autoplot-mapping, fig.alt = "autoplot() with an x-axis override and a color mapping", results = "hide"}
...
```{r autoplot-3d, screenshot.opts = list(vwidth = 800, vheight = 550, delay = 2), fig.alt = "Interactive 3-D scatter with the fitted regression surface (static snapshot)", results = "hide"}
...
ggplot(mtcars, aes(displ, hwy, color = drv)) +
  geom_point() +
  geom_slice(model, band = "cyl", predict_vars = list(hp = c(4, 7))) +
  geom_slice_text()
```
