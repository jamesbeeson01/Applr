...
(`slice_2d()`), interactive slider exploration (`slice_explore()`), 3-D
...
``` r
# Or explore: one slider per predictor the plot doesn't show, moving
# the slice in real time (also reachable as slice_explore(model))
autoplot(lm(mpg ~ disp + hp + wt, data = mtcars), sliders = TRUE)
```

<img src="man/figures/README-autoplot-sliders-1.png" alt="Interactive slider view: scatter with a slice line and one slider per hidden predictor (static snapshot)"  />

*(static snapshot — in the real widget the `hp` and `wt` sliders drag
smoothly, moving the line to that slice of the model as you go. Each
slider starts at the value the static plot would impute, so the starting
view is the plot above. `slider_breaks = list(hp = c(66, 150, 335))`
gives deliberate steps instead; `sliders = factor(cyl)` steps a numeric
predictor through its observed values; see `?slice_explore`.)*
